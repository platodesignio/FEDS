...
export function generateReport(
  ...
  if (ecoScores) {
    const eco = ecoScores
    const efdcr = Math.round(eco['E-FDCR'])
    const ebdcr = Math.round(eco.EBDCR)
    const ebde  = Math.round(eco.EBDE)
    const epbtm = Math.round(eco['EP-BTM'])

    add(
      '26. Ecological Bio-Divisional Correctness Analysis',
      '26. 生態的生命分割矯正分析',
      `Ecological Bio-Divisional Correctness Rate (EBDCR): ${ebdcr}/100. Ecological Bio-Dialectical Efficacy (EBDE): ${ebde}/100. ` +
      `EBDCR measures whether "${target}" respects ecological-biological boundaries — i.e., whether the system's operations can be reconstructed as consistent with bio-divisional freedom. ` +
      `EBDCR is a composite of positive ecological generation (EGR: ${f(ms.EGR ?? 0)}), bio-divisional efficacy (BDER: ${f(ms.BDER ?? 0)}), planetary renewal capacity index (PRCI), biodiversity generativity rate (BGR: ${f(ms.BGR ?? 0)}), global democratic re-audit rate (GDRR), and habitat generativity rate (HGR), minus ecoRiskPenalty (${f(eco.ecoRiskPenalty)}). ` +
      `Score of ${ebdcr} is ${scoreLabel(ebdcr)} — indicating the system ${ebdcr >= 60 ? 'shows measurable ecological-biological alignment' : ebdcr >= 40 ? 'has partial ecological alignment with significant correction gaps' : 'fails to meet ecological-biological correctness thresholds and requires structural ecological reform'}. ` +
      `EBDE measures the depth of ecological dialectical engagement — whether the system actively generates ecological dialectical energy rather than merely avoiding ecological harm. EBDE score: ${ebde} (${scoreLabel(ebde)}).`
    )

    add(
      '27. Ecological Freedom-Evolution Dialectical Correctness Analysis',
      '27. 生態的自由進化弁証法的矯正分析',
      `Ecological Freedom Dialectical Correctness Rate (E-FDCR): ${efdcr}/100. ` +
      `E-FDCR is the highest-order ecological synthesis metric — it integrates FDCR (${f(fdcr)}), G-FDCR (${f(gfdcr)}), EBDCR (${ebdcr}), PRCI, EGR (${f(ms.EGR ?? 0)}), BDER (${f(ms.BDER ?? 0)}), GDRR, and HGR, minus ecoPlanetaryRiskPenalty (${f(eco.ecoPlanetaryRiskPenalty)}). ` +
      `This metric asks: can the freedom generated by "${target}" survive ecological scrutiny at a planetary scale? A score of ${efdcr} places this system in the ${efdcr >= 70 ? 'ecologically correct freedom evolution' : efdcr >= 50 ? 'conditional ecological correctness' : 'ecological freedom failure'} zone. ` +
      (eco.ecoCorrections.length > 0
        ? `Ecological correction rules were triggered: ${eco.ecoCorrections.join('; ')}. These corrections represent hard limits — ecological violations that override otherwise positive FDCR scores. `
        : 'No ecological correction cap rules were triggered. ') +
      (eco.ecoJudgments.length > 0
        ? `Ecological judgment categories identified: ${eco.ecoJudgments.join(', ')}.`
        : 'No specific ecological judgment categories triggered.')
    )

    add(
      '28. Eco-Planetary Burden Transfer Matrix',
      '28. 生態的惑星負担転嫁マトリクス',
      `Eco-Planetary Burden Transfer Matrix (EP-BTM): ${epbtm}/100. ` +
      `EP-BTM measures the degree to which "${target}" transfers ecological burdens across spatial and temporal axes — from present actors to future generations, from Global North to Global South, from human-centered systems to non-human biological systems. ` +
      `EP-BTM components: Burden Transfer Rate (BTR: ${f(ms.BTR ?? 0)}), Ecological Extraction Rate (EER: ${f(ms.EER ?? 0)}), Carbon Debt Rate (CDR), Supply Chain Displacement Rate (SCDR), Finance Extraction Rate (FER), Carbon Infrastructure Rate (CIR), Data Center Resource Rate (DCR), Planetary Pressure Rate (PPR). ` +
      `A score of ${epbtm} indicates ${epbtm > 65 ? 'significant ecological planetary burden transfer — the system is offloading ecological costs onto external systems, future generations, or the non-human biosphere' : epbtm > 40 ? 'moderate ecological burden transfer with identifiable correction pathways' : 'relatively contained ecological burden transfer — system does not show evidence of large-scale ecological cost displacement'}. ` +
      `Note: EP-BTM is a risk metric — lower scores are preferable.`
    )

    add(
      '29. Spatial-Planetary Simulation Notes',
      '29. 空間惑星シミュレーション注記',
      `Spatial-Planetary Simulation Theater produces three simulation layers from this audit data: (1) Planetary View — a globe-scale visualization of burden arcs across 11 world regions, colored by arc type (climate, supply chain, labor, waste, data, financial). At current settings: EBDCR ${ebdcr}, E-FDCR ${efdcr}. ` +
      `(2) Urban Systems View — an 8×6 city block grid simulation of how FDCR metrics manifest in urban spatial distributions. Key urban-spatial risks detected: CFR ${f(ms.CFR ?? 0)} (classification fixation in spatial systems), RBR ${f(ms.RBR ?? 0)} (re-entry blockage in urban mobility), EIR (lifeworld spatial encroachment). ` +
      `(3) Institutional Flow View — a node-link diagram of the decision-processing pipeline. Key flow risks: MSJR ${f(ms.MSJR ?? 0)} (managerial self-justification at bureaucratic gates), DRR ${f(ms.DRR ?? 0)} (democratic re-audit access). ` +
      `All simulation views support scenario comparison (Reform / Current / Managerial) and time horizon projection (Now → Future). Simulation findings should be treated as analytical heuristics, not predictive models.`
    )

    add(
      '30. 3D Scenario Interpretation',
      '30. 3Dシナリオ解釈',
      `Three scenarios were simulated against the "${target}" audit data: ` +
      `(A) Reform Scenario: Risk metrics reduced by 30–35%, positive metrics enhanced by 25–30%. Under reform trajectory, E-FDCR would project to approximately ${Math.min(100, Math.round(efdcr * 1.28))} and EP-BTM burden transfer would reduce to approximately ${Math.max(0, Math.round(epbtm * 0.65))}. This scenario requires active structural correction in: re-entry capacity, democratic re-audit infrastructure, and ecological penalty reduction. ` +
      `(B) Current Trajectory: No change from computed scores. E-FDCR: ${efdcr}, EP-BTM: ${epbtm}, EBDCR: ${ebdcr}. System continues along its current directional vector. ` +
      `(C) Managerial Escalation Scenario: Risk metrics amplified by 25–35%, positive metrics suppressed by 20–25%. Managerial escalation would project E-FDCR to approximately ${Math.max(0, Math.round(efdcr * 0.72))} and EP-BTM to approximately ${Math.min(100, Math.round(epbtm * 1.35))}. This scenario represents entrenchment of existing correctness failures and warrants immediate corrective audit escalation. ` +
      `Time horizon projections: over a 10-year horizon, if no structural reform occurs, ecological risk accumulation (EER trajectory, PRCI depletion, CDR compounding) is projected to push E-FDCR into the ecological freedom failure zone (below 40).`
    )
  }
